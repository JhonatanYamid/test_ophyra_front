...
function App() {
  ...
  useEffect(() => {
    const getProducts = async () => {
      const url = `http://localhost/test_ophyra_back/index.php/products/list`;
      let jsonData = {
        start: page,
        limit: 5
      }
  ...
  const sendProduct = e => {
    e.preventDefault();
    let url = 'http://localhost/test_ophyra_back/index.php/products/insert'
    let jsonData = {
    ...
    if (change === 'update') {
      ...
      if (window.confirm(text) === true) {
        url = 'http://localhost/test_ophyra_back/index.php/products/update'
        jsonData = {
          id: data.id,
          price: parseInt(data.price),
          description: data.description,
